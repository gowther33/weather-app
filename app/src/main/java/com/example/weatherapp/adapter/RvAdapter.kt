...
class RvAdapter(private val forecastArray: ArrayList<ForecastData>) : RecyclerView.Adapter<RvAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val currentIem = forecastArray[position]
        holder.binding.apply {
            val imageIcon = currentIem.weather[0].icon
            val imageUrl = "https://openweathermap.org/img/w/$imageIcon.png"
            Picasso.get().load(imageUrl).into(imgItem)

            tvItemTemp.text = "${currentIem.main.temp}"
            tvItemStatus.text = currentIem.weather[0].description
            tvItemTime.text = displayTime(currentIem.dt_txt)
        }
    }
